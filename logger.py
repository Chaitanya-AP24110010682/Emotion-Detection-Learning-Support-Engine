...
# ── Bridge Code for app.py (Ensure this is at the absolute bottom of the file) ──
import os
import pandas as pd
import csv
...
class AppLoggerBridge:
    ...
    def log_interaction(self, *args, **kwargs):
        # 1. Capture the true values chosen by the app script
        user_text = kwargs.get('user_input') or kwargs.get('text') or (args[0] if len(args) > 0 else "")
        emotion = kwargs.get('predicted_emotion') or kwargs.get('emotion') or (args[1] if len(args) > 1 else "Joy")
        confidence = kwargs.get('confidence') or (args[2] if len(args) > 2 else 0.85)
        response = kwargs.get('ai_response') or kwargs.get('response') or (args[3] if len(args) > 3 else "")
        model_used = kwargs.get('model_used') or "bert"

        log_filepath = "data/interactions_log.csv"
        
        # 2. Directly append the true values to the CSV to defeat the hardcoded "Joy" bug
        try:
            ...
            with open(log_filepath, mode='a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                if not file_exists:
                    # Write clean headers matching what the chart expects
                    writer.writerow(['user_input', 'predicted_emotion', 'confidence', 'ai_response', 'model_used'])
                writer.writerow([user_text, emotion, confidence, response, model_used])
            print(f"[LOGGER SUCCESS] Logged true emotion: {emotion}")
            return True
        except Exception as e:
            print(f"[LOGGER EXCEPTION] Manually fallback: {e}")
            
        # Call original function just in case other backup scripts rely on it
        try:
            return log_interaction(user_text, emotion, confidence, response)
        except Exception:
            pass
    ...
    def summary_stats(self):
        ...
        try:
            if os.path.exists(log_filepath) and os.path.getsize(log_filepath) > 0:
                ...
                # Ensure columns exist to avoid chart errors
                for col in ['predicted_emotion', 'confidence', 'model_used']:
                    if col not in df.columns:
                        df[col] = "bert" if col == "model_used" else (0.85 if col == "confidence" else "Joy")
                return df
        except Exception:
            pass
